<template>
	...
	<view class="popup">
		...
		<view @touchmove.stop.prevent="moveHandle" :class="'sku ' + (showModal==true ? 'shows':'')"
			...
			<view class="sku_top">
				<image v-if="currentSku.imageUrl!=null" :src="currentSku.imageUrl[0].imageUrl" class="top_img"></image>
				<view class="sku_title">
					{{nowList.title}}
				</view>
				<view class="moneys">
					<!-- 这里的价格是选择完规格后计算的价格 -->
					￥{{(Number(currentSku.price) * number).toFixed(2)}}
				</view>
				<view class="kucun">
					<!-- 这里的价格是选择完规格后计算的库存 -->
					库存: {{currentSku.storage}}
				</view>
			</view>
		...
		methods: {
			...
			onsubmit(value) {
				...
				if (value == 'add') { //如果是添加购物车
					var temp = {
						goods_id: this.skuList.goodsId,
						count: this.number
					}
					console.log(temp)
					this.request(this.server_url + 'shopping/insert', temp, 'POST').then((res) => {
						console.log(res);
						if(res.code==0)
							uni.showToast({
								title: '加入购物车成功 !',
					...
					goods.push(datas)
					console.log(datas);
					uni.setStorageSync('confirmOrder',datas);
					uni.navigateTo({ //提交订单
