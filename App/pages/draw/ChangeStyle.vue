	...
	<view style="display: flex;flex-direction: column;background-color: #f6f6f6;">
		...
		<view style="width: 100%;height: 15vh;display: flex;">
			...
			<view v-else style="display: flex;flex-direction: row;justify-content: space-between;align-items: center;width: 100%;">
				...
				<scroll-view scroll-x="true" style="width: 80%;margin-left: 1%;margin-right: 1%;display: flex;flex-direction: row;align-items: center;">
				...
				<view style="background-color: #3b5bfe;width: 70px;margin-top: 10px;height: 30px;display: flex;align-items: center;justify-content: center;margin-left: 5px;border-radius: 5px;margin-right: 5px;flex-shrink: 0;font-size: 16px;color: white" @click="start_qiehuan">
					切换
				</view>
			</view>
	...
	export default {
		...
		methods: {
			start_qiehuan(){
				...
				var temp = {
					style:this.StyleMenu[this.StyleNow],
					ChangeStyle:[],
					now_room:this.now_room
				}
					
				for(var i=0;i<this.house.Room.length;i++){
					if(this.house.Room[i].checked)
						temp.ChangeStyle.push(this.house.Room[i].roomId);
				}
				console.log(temp);
				window.location.href = "uniwebview://ChangeStyle"+JSON.stringify(temp);
				uni.showLoading({
					title:'切换中'
				});
				setTimeout(function(){
			...
			init_check(){
				for(var i=0;i<this.house.Room.length;i++){
					this.house.Room[i] = {...this.house.Room[i],checked:false};
				}
			},
			get_area() {
				// 获取房屋面积
			...
			touchstart(e) {
				...
				// 打标签操作
				var roomId = this.GetRoomId(e.touches[0]);
				if(roomId!=-1) this.house.Room[roomId].checked = true;
			...
			FillRoom() {
				...
				for(var j=0;j<this.house.Room.length;j++){
					console.log(j+" "+this.house.Room[j].checked);
					if(this.house.Room[j].checked){
						// console.log("选中："+i)
						this.ctx.beginPath();
						
						var temp = this.house.Room[j].point;
